...
class CerebrasPRReviewer:
    ...
    def review_pr_file(self, file_path: str, content: str, changed_lines: list[int]) -> dict:
        ...
        # Chỉ review những dòng thay đổi
        lines = content.split('\n')
        reviewed_lines = {}
        
        for line_num in changed_lines:
            if line_num > 0 and line_num <= len(lines):
                # Review từng dòng thay đổi
        ...
        return {
            ...
            "approved": len(reviewed_lines) == 0,
        }

    def _review_line(self, file_path: str, line_num: int, line_content: str) -> dict:
        """Review một dòng code"""
        prompt = f"""Review this single line of code from file {file_path} at line {line_num}:

Code line:
{line_content}

Provide ONLY a concise review in JSON format:
{{
  "line": {line_num},
  "file": "{file_path}",
...
Return ONLY valid JSON. If no issues, return {{"line": {line_num}, "comments": [], "severity": "none"}}
"""
        ...
        try:
            ...
            if response.status_code != 200:
                return {"line": line_num, "comments": [], "severity": "error"}
