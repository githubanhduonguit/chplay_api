"""In-app async queue for review jobs.

A thin typed wrapper around ``asyncio.Queue`` used to decouple review
creation from bot reply generation. The queue lives inside the
application process — no external infrastructure (Celery/Redis/RQ).
"""

from __future__ import annotations

import asyncio

from app.services.queue.schemas import ReviewJob


class ReviewJobQueue:
    """Async queue for ``ReviewJob`` items.

    Args:
        maxsize: Maximum number of jobs the queue can hold before
            ``enqueue`` blocks. Defaults to 1000.
    """

    def __init__(self, maxsize: int = 1000) -> None:
        self._queue: asyncio.Queue[ReviewJob] = asyncio.Queue(
            maxsize=maxsize
        )

    async def enqueue(self, job: ReviewJob) -> None:
        """Put a job into the queue.

        Args:
            job: The ReviewJob to enqueue.
        """
        await self._queue.put(job)

    async def dequeue(self) -> ReviewJob:
        """Get the next job from the queue, blocking until one is available.

        Returns:
            The next ReviewJob.
        """
        return await self._queue.get()

    def size(self) -> int:
        """Get the current number of jobs in the queue.

        Returns:
            The current queue size.
        """
        return self._queue.qsize()


# Module-level singleton queue shared across the application.
review_job_queue = ReviewJobQueue()
